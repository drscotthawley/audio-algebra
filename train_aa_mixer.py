...
def train_aa_model(debug=False):   
    ...
    aa_model, opt, train_dl, scheduler = accelerator.prepare(aa_model, opt, train_dl, scheduler)
    ...
    while (epoch < max_epochs) or (max_epochs < 0):  # training loop
        ...
        with tqdm(train_dl, unit="batch", disable=not accelerator.is_main_process) as tepoch:
            for batch in tepoch:   # train
                ...
                # vicreg: 1. invariance
                zsum, zmix, archive = do_mixing(stems, faders, given_model, accelerator.unwrap_model(aa_model), device, debug=debug)
                ...
                if accelerator.is_main_process:
                    if  False and step % viz_demo_every == 0:
                         log_dict = aa_demo(given_model, accelerator.unwrap_model(aa_model), log_dict, zsum, zmix, step)
